...
type MemberQuery struct {
	...
	withEmergencycontact *EmergencyContactQuery
...
// QueryEmergencycontact chains the current query on the "emergencycontact" edge.
func (mq *MemberQuery) QueryEmergencycontact() *EmergencyContactQuery {
	query := (&EmergencyContactClient{config: mq.config}).Query()
...
func (mq *MemberQuery) Clone() *MemberQuery {
	...
	return &MemberQuery{
		...
		withEmergencycontact: mq.withEmergencycontact.Clone(),
...
// WithEmergencycontact tells the query-builder to eager-load the nodes that are connected to
// the "emergencycontact" edge. The optional arguments are used to configure the query builder of the edge.
func (mq *MemberQuery) WithEmergencycontact(opts ...func(*EmergencyContactQuery)) *MemberQuery {
	query := (&EmergencyContactClient{config: mq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	mq.withEmergencycontact = query
	return mq
}
...
func (mq *MemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Member, error) {
	...
	if query := mq.withEmergencycontact; query != nil {
		if err := mq.loadEmergencycontact(ctx, query, nodes, nil,
			func(n *Member, e *EmergencyContact) { n.Edges.Emergencycontact = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MemberQuery) loadEmergencycontact(ctx context.Context, query *EmergencyContactQuery, nodes []*Member, init func(*Member), assign func(*Member, *EmergencyContact)) error {
	...
	query.Where(predicate.EmergencyContact(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(member.EmergencycontactColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.member_emergencycontact
		if fk == nil {
			return fmt.Errorf(`foreign-key "member_emergencycontact" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "member_emergencycontact" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
